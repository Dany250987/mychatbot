...
async function saveVoiceReminder(reminderData) {
  ...
  /*
 * En Android programamos la notificación nativa
 * correspondiente al recordatorio creado por voz.
 */
  if (
  ...
  */
  ...
  /*
  * Marcamos la actividad recién creada para que,
  * al renderizar la lista, pueda ser localizada.
  */
  if (
...
function fillManualReminderFormFromVoice(reminder) {
  ...
  if (dueDateInput) {
    ...
    dueDateInput.dataset.autoGenerated = "true";
  }
...
function getVoiceReminderLastDayOfMonth(dateValue) {
  if (!dateValue) {
    return "";
  }
